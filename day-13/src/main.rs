...
fn check_solution(t: usize, offsets: &[BusOffset]) -> bool {
    offsets.iter().all(|bus| (t + bus.offset) % bus.id == 0)
}
...
fn find_solution(start_t: usize, buses: &[BusId]) -> usize {
    ...
    let mut max_id_offset = 0;
    let mut max_id = 0;
    for bus in &offsets {
        if bus.id > max_id {
            max_id = bus.id;
            max_id_offset = bus.offset;
        }
    }

    let mut t = start_t + max_id - ((start_t + max_id) % max_id) - max_id_offset;
    // let mut t = max_id - max_id_offset;
    loop {
